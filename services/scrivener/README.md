...
`POST /ingestion/resolve`, `POST /ingestion/series`, and `POST /releases/sync`
must send the same value in the `X-Scrivener-API-Key` header. Those endpoints
fail closed when no service key is configured.
